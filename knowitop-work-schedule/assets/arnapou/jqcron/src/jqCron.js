...
var jqCronDefaultSettings = {
	...
	numeric_zero_pad_mins: true,
	numeric_zero_pad_time_min: true,
	numeric_zero_pad_time_hour: true,
	numeric_zero_pad_dom: false,
	default_period: 'day',
	default_value: '',
...
(function($){
	...
	function jqCron(settings) {
		...
		this.getText = function(key) {
			...
			if(typeof(text) == "string" && text.match('<br')){
				text = text.replace(/(<br *\/>)/gi, '</span><br /><span class="jqCron-text">');
		...
		this.init = function(){
			...
			for(i=0, list=settings.minutes; i<list.length; i++){
				_selectorMins.add(list[i], settings.numeric_zero_pad_mins ? ("00" + list[i]).slice(-2) : list[i]);
			}
			...
			for(i=0, list=settings.hours, labelsList=settings.hour_labels; i<list.length; i++){
				_selectorTimeH.add(list[i], settings.numeric_zero_pad_time_hour ? ("00" + labelsList[i]).slice(-2) : labelsList[i]);
			}
			...
			for(i=0, list=settings.minutes, labelsList=settings.minute_labels; i<list.length; i++){
				_selectorTimeM.add(list[i], settings.numeric_zero_pad_time_min ? ("00" + labelsList[i]).slice(-2) : labelsList[i]);
			}
			...
			for(i=0, list=settings.monthdays, labelsList=settings.monthday_labels; i<list.length; i++){
				_selectorDom.add(list[i], settings.numeric_zero_pad_dom ? ("00" + labelsList[i]).slice(-2) : labelsList[i]);
			}
