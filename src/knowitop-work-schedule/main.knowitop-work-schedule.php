...
spl_autoload_register(function ($sClass) {
	if (strpos($sClass, 'Knowitop\\') === 0)
	{
		$sFileName = substr($sClass, strlen('Knowitop\\')).'.php';
		require_once 'classes/'.$sFileName;
	}
});
 ...
 */
abstract class _WorkSchedule extends Ticket
{
	/** @var string – formatted current time to use in all computations */
	protected $sNow;

	/**
	 * _WorkSchedule constructor.
	 *
	...
	public function GetWorkOrderCreateInterval(): int
	{
		...
		$iMaxValue = 2678400; // Max interval 2678400 seconds (31 days)
	...
	{
		if (empty($sCurrentDate)) $bIncludeCurrentDate = true;
	...
	public function CreateNextWorkOrder(): ?WorkOrder
	{
		...
		$oWorkOrder->Set(WO_END_DATE_ATTR,
			AttributeDateTime::GetAsUnixSeconds($sNextWOStartDate) + $oWorkOrderTemplate->Get('wo_duration'));
